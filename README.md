...
1. Клонируйте репозиторий на свой компьютер:
    ...
    ```bash
    git clone https://github.com/kda99/taski-docker.git
    ```
    ```bash
    cd taski_final
    ```
...
1.  Замените username на ваш логин на DockerHub:
    ...
    ```bash
    cd backend
    docker build -t kda99/taski_backend:latest .
    cd frontend
    docker build -t kda99/taski_frontend:latest .
    cd gateway
    docker build -t kda99/taski_gateway:latest . 
    ```

2. Загрузите образы на DockerHub:

    ```bash
    docker push kda99/taski_backend:latest
    docker push kda99/taski_frontend:latest
    docker push kda99/taski_gateway:latest
    ```
...
1. Подключитесь к удаленному серверу
    ...
    ```bash
    ssh -i /home/ххххх/yyyyyy/555/yc-ХХХХ yc-user@158.160.0.10
key ХХХХХХ
...
server {
    ...
    listen 443 ssl; # managed by Certbot
    ssl_certificate /etc/letsencrypt/live/ХХХ/fullchain.pem; # managed by Certbot
    ssl_certificate_key /etc/letsencrypt/live/ХХХ/privkey.pem; # managed by Certbot
    include /etc/letsencrypt/options-ssl-nginx.conf; # managed by Certbot
    ssl_dhparam /etc/letsencrypt/ssl-dhparams.pem; # managed by Certbot
}
```
...
services:
  ...
  backend:
    image: ХХХ/taski_backend # Docker Hub username / username on Docker image
    env_file: .env
    volumes:
      - static_volume:/backend_static
  frontend:
    image: ХХХ/taski_frontend  # Download from Docker Hub
    env_file: .env
    command: cp -r /app/build/. /frontend_static/
    volumes:
      - static_volume:/frontend_static
  gateway:
    image: ХХХ/taski_gateway  # Download from с Docker Hub
    env_file: .env
    volumes:
